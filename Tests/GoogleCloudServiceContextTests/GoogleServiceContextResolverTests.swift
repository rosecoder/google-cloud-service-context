...
@Suite(.serialized) struct GoogleServiceContextResolverTests {
    ...
    @Test func shouldFailIfNoMetadataServer() async throws {
        await #expect(throws: Error.self) {
            _ = try await resolver.fetchFromMetadataServer(resource: "project/id")
        }
    }
    ...
    @Test func shouldResolve() async throws {
        ...
        let projectID = try await resolver.fetchFromMetadataServer(resource: "project/id")
        #expect(projectID == "test-project-id")

        let projectID2 = try await resolver.fetchFromMetadataServer(resource: "project/id")
